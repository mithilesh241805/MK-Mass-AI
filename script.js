...
const AI = {
  ...
  openai:  { url: 'https://api.groq.com/openai/v1/chat/completions',     model: 'deepseek-r1-distill-llama-70b' },
  gemini:  { url: 'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent' },
  claude:  { url: 'https://api.groq.com/openai/v1/chat/completions',     model: 'qwen-qwq-32b' }
};
...
const KEY_FORMAT = {
  ...
  openai: /^gsk_[A-Za-z0-9]{20,}$/,
  gemini: /^AIzaSy[A-Za-z0-9\-_]{25,}$/,
  claude: /^gsk_[A-Za-z0-9]{20,}$/
};
...
- ## Signature Chef Tip: One secret "MK Supreme" trick.`;
  ...
  if (provider === 'gemini') {
    ...
    if (!r.ok) { const err = await r.json().catch(()=>({})); throw new Error(`${r.status}: ${err?.error?.message || 'Cerebras error'}`); }
    const d = await r.json();
